...
internal static class ZwCadUiContext
{
        ...
        TimeSpan timeout)
    {
        ...
        if (!done.Wait(timeout))
            throw new TimeoutException($"ZwCAD UI context timed out after {timeout.TotalSeconds:F0}s.");
    ...
    private static T RunViaApplicationContext<T>(Func<T> action, TimeSpan timeout)
    {
        ...
        while (Environment.TickCount64 < deadline)
        {
            ...
            if (state == 2)
                throw error ?? new InvalidOperationException("ZwCAD application context failed.");

            Thread.Sleep(20);
        }

        throw new TimeoutException($"ZwCAD application context timed out after {timeout.TotalSeconds:F0}s.");
    }
}
