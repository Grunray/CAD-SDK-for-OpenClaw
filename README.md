...
| M6 — 文档与发版 | 双发(npm + ClawHub),写"如何安装插件 + netload + 启动 skill"的对接手册;按 [辉火云管家插件铁律](https://www.huo15.com) §11 自检清单跑一遍 |
...
| 路径 | 难度 | 预估工作量 | 适用场景 |
|---|---|---|---|
| **直接复用 `puran-water/autocad-mcp` + 辉火云管家 MCP 客户端** | 🟢 1/5 | 0.5 天 — 只需配置 `~/.openclaw/mcp.json` 一条 entry | 可以接受 **Python + AutoLISP** 技术栈,且只需要 zoom + 基本 CRUD;模糊查文字要在 Python 端补 LISP 函数 |
...
**结论**:如果**死磕 .NET 8 单栈**,gileCAD R25 + ASP.NET Core Minimal API + skill wrapper 是最优解,3-5 天内 MVP;如果**愿意混栈**,直接挂 puran-water 的 MCP 是最快验证路径,半天就能让辉火云管家调通 zoom。
