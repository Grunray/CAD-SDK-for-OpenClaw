...
# 在填默认值之前记住用户是否显式设置了端口（显式端口优先于 runtime 发现文件）
COALCLAW_HTTP_PORT_SET="${COALCLAW_HTTP_PORT:+1}"
...
COALCLAW_PLUGIN_DLL="${COALCLAW_PLUGIN_DLL:-$REPO_ROOT/plugin/src/CoalClaw.ZwCAD.Plugin/bin/Debug/net8.0/CoalClawZwCADPlugin.dll}"

export COALCLAW_CAD_HOST COALCLAW_PLATFORM COALCLAW_CAD_EXE COALCLAW_PLUGIN_DLL
export COALCLAW_HTTP_PORT COALCLAW_HTTP_PORT_SET COALCLAW_PING_WAIT_SEC COALCLAW_PING_INTERVAL_SEC
